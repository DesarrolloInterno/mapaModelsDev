...
const typeDocumentsEmployeesMutations = {
    ...
    CREATE_TYPE_DOCUMENTS_EMPLOYEES: async (_: any, {input}: any) => {
        ...
        try {
            const newRecord = await typeDocumentsEmployees.query("exec create_typeDocumentsEmployees @name = '"+name+"', @description = '"+description+"', @userLoggedIn = '"+userLoggedIn+"', @idEnterprise = '"+ idEnterprise +"'");
    ...
    UPDATE_TYPE_DOCUMENTS_EMPLOYEES: async (_: any, {input}: any) => {
        const {idTypeDocumentsEmployees, name, description, userLoggedIn, idEnterprise} = input;
        try {
            const newRecord = await typeDocumentsEmployees.query("exec update_typeDocumentsEmployees @idTypeDocumentsEmployees = " + idTypeDocumentsEmployees + ", @name = '"+name+"', @description = '"+description+"', @userLoggedIn = '"+userLoggedIn+"', @idEnterprise = '"+ idEnterprise + "'");
            return newRecord[0].message;
        } catch (error) {
            console.log(error);
        }
